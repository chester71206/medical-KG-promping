...
if __name__ == "__main__":
    ...
    # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        #chinese_question="我有胰臟炎病史，我下背痛超過一個月，請問是甚麼問題"
        ...
        # re1 現在是 'r<CLS>.*<SEP>The extracted entities are (.*?)<EOS>''
        # 根據 Preprocessing 裡的 regex 調整
